//menuBtn(tip, keyCombination, path)
function castKeyMap(keyMap) {
    const map = {
        "products-view": {
            cast: "Productos",
...
window.onload = async () => {
    ...
    for (const key of Object.keys(castedKeyMap)) {
        const k = castedKeyMap[key]
        const $btn = menuBtn(key, k.keyCombination, k.path)
        $menu.appendChild($btn)
    }
    window.parent.menu.setButtons($menu.children)
