class ProductView {
    ...
    addProductUnit(units) {
        ...
        $container.className =
            "rounded border w-full grid grid-cols-3 gap-4 p-2 border-slate-600"
        const $select = select("", "Unidad", units)
        ...
        return { buy, profit, descount, sell }
    }

    removeProductUnit() {
        const child = this.$units.lastChild
